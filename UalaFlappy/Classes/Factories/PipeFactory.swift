...
//            }
    ...
    private static func standardPipeParts(for sceneSize: CGSize) -> PipeParts? {
        ...
        guard let unwrappedPipeBottom = pipeBottom else {
            debugPrint(#function + " could not contruct PipeNode instance for bottom pipe")
            return nil
        }
        ...
        // Top pipe node
        var topHeight = sceneSize.height - (pipeBottom?.size.height)! - threshold.size.height
        ...
        guard let unwrappedPipeTop = pipeTop else {
            debugPrint(#function + " could not contruct PipeNode instance for top pipe")
            return nil
        }
