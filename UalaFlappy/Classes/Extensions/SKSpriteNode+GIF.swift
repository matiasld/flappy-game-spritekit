...
extension SKSpriteNode {
    ...
    convenience init(asset: FlappyAsset, size: CGSize = .zero) {
        guard let image = FlappyImage(named: asset) else {
            self.init(texture: nil, color: .clear, size: .zero)
            return
        }
    ...
    convenience init(named name: String, size: CGSize = .zero) {
        guard let image = FlappyImage(named: name) else {
            self.init(texture: nil, color: .clear, size: .zero)
            return
        }
        let texture = SKTexture(image: image)
