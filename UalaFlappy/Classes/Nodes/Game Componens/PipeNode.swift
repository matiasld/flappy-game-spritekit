...
class PipeNode: SKSpriteNode {
    ...
    init?(textures: (pipe: String, cap: String), of size: CGSize, side: IsTopPipe) {
        ...
        guard let texture = FlappyImage(named: textures.pipe)?.cgImage else {
            return nil
        }
        ...
        let backgroundTexture = SKTexture(image: unwrappedTiledBackground)
        
        let pipe = SKSpriteNode(texture: backgroundTexture)
        pipe.zPosition = 1
        
        let cap = SKSpriteNode(named: textures.cap)
        ...
        let capWidth = pipe.size.width + pipe.size.width / 6
        cap.size = CGSize(width: capWidth, height: capWidth / 3.5)
        cap.zPosition = 5
        pipe.addChild(cap)
